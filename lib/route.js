...
import { loginWithBearerToken } from "./oauth";
...
Route.prototype._callEndpoint = async function (endpointContext, endpoint) {
    ...
    if (auth.success) {
        if (this._roleAccepted(endpointContext, endpoint)) {
            ...
            return DDP._CurrentInvocation.withValue(invocation, Meteor.wrapAsync(async function (done) {
                ...
                if (_.isFunction(onAction)) {
                    ...
                    await onAction.call(endpointInvocation);
                }

                let returnValue = await endpoint.action.call(endpointInvocation);

                if (_.isFunction(onReturn)) {
                    // global onReturn function, this allows to capture the return value and manupulate before
                    // sending to the client
                    returnValue = await onReturn.call(endpointInvocation, returnValue) || returnValue;
                }
...
Route.prototype._authenticate = async function (endpointContext) {
    if (endpointContext.request?.headers?.authorization) {
        let auth;
        // oauth authorization header using oauth2 server package
        const header = endpointContext.request.headers.authorization;
        const parts = header.split(' ');
        const scheme = parts[0];
        const credentials = parts[1];
        if (/^Bearer$/i.test(scheme)) {
            try {
                auth = await loginWithBearerToken(credentials);
                endpointContext.user = auth.user;
                endpointContext.userId = auth.user._id;
                endpointContext.auth = auth;
                return {
                    success: true,
                    data: auth
                };
